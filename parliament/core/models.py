...
POL_LOOKUP_URL = 'http://www.parl.gc.ca/MembersOfParliament/' \
    'ProfileMP.aspx?Key=%d&Language=E'
...
class InternalXref(models.Model):
    ...
    # edid_postcode -- the EDID -- which points to a riding,
    #     but is NOT the primary  key -- for a postcode
    schema = models.CharField(max_length=15, db_index=True)

    def __unicode__(self):
        return _(u"%(schema)s: %(text)s %(int)s for %(target)s") % {
            schema: self.schema,
...
class PartyManager(models.Manager):
    ...
    def get_by_name(self, name):
        ...
        if len(x) == 0:
            ...
            raise Exception(
                _("More than one party matched %s") % name.strip().lower()
            )
        else:
...
class Party(models.Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # set a flag to save the name to the alternate-names table.
    ...
    def add_alternate_name(self, name):
        ...
        if len(x) == 0:
            ...
            if x[0].target_id != self.id:
                raise Exception(
                    _("Name %s already points to a different party") \
                    % name.strip().lower()
                )
...
class Person(models.Model):
    ...
    name = models.CharField(max_length=100)
    name_given = models.CharField(_("Given name"), max_length=50, blank=True)
    name_family = models.CharField(_("Family name"), max_length=50, blank=True)
...
class PoliticianManager(models.Manager):
    ...
    def elected(self):
        """Returns a QuerySet of all politicians that were
        once elected to office."""
        return self.get_query_set().annotate(
            electedcount=models.Count('electedmember'))\
                .filter(electedcount__gte=1)

    def never_elected(self):
        """Returns a QuerySet of all politicians that were
        never elected as MPs.
    ...
    def current(self):
        ...
        return self.get_query_set().filter(
            ...
            electedmember__start_date__isnull=False
        ).distinct()
    ...
    def filter_by_name(self, name):
        ...
        return [i.politician for i in
            PoliticianInfo.sr_objects.filter(
                schema='alternate_name',
                value=parsetools.normalizeName(name)
            )]
    ...
    def get_by_name(
        ...
        """ Return a Politician by name. Uses a bunch of methods to try
        and deal with variations in names. If given any of a session,
        riding, election, or party, returns only politicians who match.
        If given session and optinally riding, tries to match the name
        more laxly.

        saveAlternate: If we have Thomas Mulcair and we match,
        via session/riding, to Tom Mulcair, save Tom in the alternate
        names table strictMatch: Even if given a session, don't try
        last-name-only matching."""

        # Alternate names for a pol are in the InternalXref table.
        # Assemble a list of possibilities
        ...
        if len(poss) >= 1:
            ...
            if session or riding or party:
                ...
                for p in poss:
                    ...
                    if len(members) >= 1:
                        if result:
                        # we found another match on a previous
                ...
                raise Exception(
                    _("Election not implemented yet in Politician get_by_name")
                )
            else:
                # No extra criteria -- return what we got
                # (or die if we can't disambiguate)
                if len(poss) > 1:
                    raise Politician.MultipleObjectsReturned(name)
                else:
        ...
        if session and not strictMatch:
            # We couldn't find the pol, but we have the session and riding,
            ...
            if match:
                ...
                pols = self.get_query_set().filter(
                    name_family=lastname,
                    electedmember__sessions=session
                ).distinct()
                if riding:
                ...
                if len(pols) > 1:
                    if riding:
                        raise Exception(
                            _("DATA ERROR: There appear to be two " \
                            "politicians with the same last name elected " \
                            "to the same riding from the same session... " \
                            "%(lastname)s %(session)s %(riding)s") % {
                                lastname: lastname,
                                session: session,
                                riding: riding
                            }
        ...
        raise Politician.DoesNotExist(
            _("Could not find politician named %s") % name
        )

    def get_by_parlinfo_id(self, parlinfoid, session=None):
        PARLINFO_LOOKUP_URL = 'http://www2.parl.gc.ca/parlinfo/Files/' \
            'Parliamentarian.aspx?Item=%s&Language=E'
        try:
            info = PoliticianInfo.sr_objects.get(
                schema='parlinfo_id',
                value=parlinfoid.lower()
            )
            return info.politician
        except PoliticianInfo.DoesNotExist:
            print _("Looking up parlinfo ID %s") % parlinfoid
            parlinfourl = PARLINFO_LOOKUP_URL % parlinfoid
            parlinfopage = urllib2.urlopen(parlinfourl).read()
            match = re.search(
              r'href="http://webinfo\.parl\.gc\.ca/MembersOfParliament/' \
              'ProfileMP\.aspx\?Key=(\d+)&amp;Language=E">MP profile',
              parlinfopage
            )
            if not match:
                raise Politician.DoesNotExist
    ...
    def get_by_parl_id(
        ...
        try:
            info = PoliticianInfo.sr_objects.get(
                schema='parl_id',
                value=unicode(parlid)
            )
            return info.politician
        except PoliticianInfo.DoesNotExist:
            if not lookOnline:
                # FIXME inconsistent behaviour:
                # when should we return None vs. exception?
                return None
            ...
            try:
                ...
                raise Politician.DoesNotExist(
                    _("Couldn't open " + (POL_LOOKUP_URL % parlid))
                )
            if soup.find(
                'table',
                id='MasterPage_BodyContent_PageContent_PageContent_pnlError'
            ):
                # print "Error page for parlid %d" % parlid
                raise Politician.DoesNotExist(
                    _("Invalid page for parlid %s") % parlid
                )
            polname = soup.find(
                'span',
                id='MasterPage_MasterPage_BodyContent_PageContent_Content' \
                '_TombstoneContent_TombstoneContent_ucHeaderMP_lblMPNameData'
            ).string
            polriding = soup.find(
                'a',
                id='MasterPage_MasterPage_BodyContent_PageContent_Content' \
                '_TombstoneContent_TombstoneContent_ucHeaderMP' \
                '_hlConstituencyProfile'
            ).string
            parlinfolink = soup.find(
                'a',
                id='MasterPage_MasterPage_BodyContent_PageContent_Content' \
                '_TombstoneContent_TombstoneContent_ucHeaderMP' \
                '_hlFederalExperience'
            )
            try:
                riding = Riding.objects.get_by_name(polriding)
            except Riding.DoesNotExist:
                raise Politician.DoesNotExist(
                    _("Couldn't find riding %s") % polriding
                )
            if session:
...
class Politician(Person):
    ...
    def alternate_names(self):
        ...
        return self.politicianinfo_set.filter(schema='alternate_name')\
    ...
    def current_member(self):
        """If this politician is a current MP, returns the corresponding
        ElectedMember object.

        Returns False if the politician is not a current MP."""
        try:
            return ElectedMember.objects.get(
                politician=self,
    ...
    def latest_member(self):
        """If this politician has been an MP,
        returns the most recent ElectedMember object.
    ...
    def latest_candidate(self):
        ...
        Returns None if we're not aware of any candidacies
        for this politician."""
        try:
            return self.candidacy_set\
                .order_by('-election__date')\
                .select_related('election')[0]
        except IndexError:
            return None
    ...
    def save_parl_id(self, parlid):
        if PoliticianInfo.objects.filter(
            schema='parl_id',
            value=unicode(parlid)
        ).exists():
            raise Exception(_("ParlID %d already in use") % parlid)
    ...
    def parlpage(self):
        try:
            return "http://www.parl.gc.ca/MembersOfParliament/" \
                "ProfileMP.aspx?Key=%s&Language=E" % self.info()['parl_id']
        except KeyError:
            return None
    ...
    def get_contact_url(self):
        if self.slug:
            return (
                'parliament.contact.views.contact_politician',
                [],
                {'pol_slug': self.slug}
            )
        return (
    ...
    @memoize_property
    def info(self):
        """Returns a dictionary of PoliticianInfo attributes
        for this politician.
        e.g. politician.info()['web_site']"""
        return dict([i for i in self.politicianinfo_set.all()\
            .values_list('schema', 'value')])

    @memoize_property
    def info_multivalued(self):
        """Returns a dictionary of PoliticianInfo attributes for
        this politician, where each key is a list of items. This allows
    ...
    def set_info(self, key, value):
        try:
            ...
            logger.error(
                "Multiple objects found for schema %s on politician %r: %r" % (
                    key,
                    self,
                    self.politicianinfo_set.filter(schema=key)\
                        .values_list('value', flat=True)
                )
            )
    ...
    def find_favourite_word(self, wordcloud=True):
        ...
        if self.current_member:
            ...
            statements = statements.filter(time__gte=datetime.datetime.now()
        ...
        self.set_info(
            'favourite_word',
            text_utils.most_frequent_word(statements)
        )
        if wordcloud:
            image = text_utils.statements_to_cloud(statements)
            path = os.path.join(
                self.WORDCLOUD_PATH,
                "%s.png" % (self.slug if self.slug else self.id)
            )
...
class SessionManager(models.Manager):
    ...
    def with_bills(self):
        return self.get_query_set()\
            .filter(bill__number_only__isnull=False)\
            .distinct()
    ...
    def get_by_date(self, date):
        return self.filter(
            models.Q(end__isnull=True) | models.Q(end__gte=date)
        ).get(start__lte=date)
...
class ElectedMemberManager(models.Manager):
    ...
    def get_by_pol(self, politician, date=None, session=None):
        if not date and not session:
            raise Exception(
                _("Provide either a date or a session to get_by_pol.")
            )
        if date:
            return self.on_date(date).get(politician=politician)
        else:
            # In the case of floor crossers,
            # there may be more than one ElectedMember
            ...
            if not len(qs):
                raise ElectedMember.DoesNotExist(
                    _("No elected member for %(poli)s, session %(sess)s") % {
                        poli: politician,
                        sess: session
                    }
                )
...
class ElectedMember(models.Model):
    ...
    def __unicode__(self):
        if self.end_date:
            return _(u"%(poli)s (%(party)s) was the member from %(riding)s " \
                "from %(start)s to %(end)s") % {
                    poli: self.politician,
                    party: self.party,
                    riding: self.riding,
            ...
            return _(u"%(poli)s (%(party)s) is the member from %(riding)s " \
                "(since %(start)s)") % {
                    poli: self.politician,
                    party: self.party,
                    riding: self.riding,
