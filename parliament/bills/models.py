...
from parliament.core.models import Session, InternalXref, \
    ElectedMember, Politician, Party
...
CALLBACK_URL = 'http://www2.parl.gc.ca/HousePublications/' \
    'GetWebOptionsCallBack.aspx?SourceSystem=PRISM' \
    '&ResourceType=Document&ResourceID=%d&language=1&DisplayMode=2'

BILL_VOTES_URL = 'http://www2.parl.gc.ca/Housebills/' \
    'BillVotes.aspx?Language=E&Parl=%s&Ses=%s&Bill=%s'

LEGISINFO_BILL_URL = 'http://www.parl.gc.ca/LegisInfo/' \
    'BillDetails.aspx?Language=%(lang)s&Mode=1&Bill=%(bill)s' \
    '&Parl=%(parliament)s&Ses=%(session)s'

PARLIAMENT_DOCVIEWER_URL = 'http://parl.gc.ca/HousePublications/' \
    'Publication.aspx?Language=%(lang)s&Mode=1&DocId=%(docid)s'


class BillManager(models.Manager):

    def get_by_legisinfo_id(self, legisinfo_id):
        """Given a House of Commons ID
        (e.g. from LEGISinfo, or a Hansard link),
    ...
    def create_temporary_bill(self, legisinfo_id, number, session):
        ...
        """
        if BillInSession.objects.filter(
            legisinfo_id=int(legisinfo_id)
        ).exists():
...
class Bill(models.Model):
    ...
    text_docid = models.IntegerField(
        blank=True,
        null=True,
        help_text=_("The parl.gc.ca document ID of the latest version " \
            "of the bill's text")
    )
    ...
    def save_sponsor_activity(self):
        if self.sponsor_politician:
            activity.save_activity(
                ...
                date=self.introduced if self.introduced \
                    else (self.added - datetime.timedelta(days=1)),
                variety='billsponsor',
            )
...
class BillInSession(models.Model):
    ...
    legisinfo_id = models.PositiveIntegerField(
        db_index=True,
        blank=True,
        null=True
    )
    ...
    def __unicode__(self):
        return _(u"%(bill)s in session %(id)s") % {
            bill: self.bill,
            id: self.session_id
        }
...
VOTE_RESULT_CHOICES = (
    ('Y', _('Passed')),  # Agreed to
    ('N', _('Failed')),  # Negatived
    ('T', _('Tie')),
)
...
class VoteQuestion(models.Model):
    ...
    def __unicode__(self):
        return _(u"Vote #%(number)s on %(date)s") % {
            number: self.number,
            date: self.date
        }
    ...
    def label_absent_members(self):
        for member in ElectedMember.objects.on_date(self.date).exclude(
            membervote__votequestion=self
        ):
            MemberVote(
                votequestion=self,
                member=member,
    ...
    def label_party_votes(self):
        """Create PartyVote objects representing the party-line vote;
        label individual dissenting votes."""
...
class MemberVote(models.Model):
    ...
    def __unicode__(self):
        return _(u'%(politician)s voted %(vote)s on %(question)s') % {
            politician: self.politician,
            vote: self.get_vote_display(),
            question: self.votequestion
        }
    ...
    def save_activity(self):
        activity.save_activity(
            self,
            politician=self.politician,
            date=self.votequestion.date
        )


VOTE_CHOICES_PARTY = VOTE_CHOICES + [('F', _("Free vote"))]
...
class PartyVote(models.Model):
    ...
    def __unicode__(self):
        return _(u'%(party)s voted %(vote)s on %(question)s') % {
            party: self.party,
            vote: self.get_vote_display(),
            question: self.votequestion
        }
