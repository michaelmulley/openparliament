...
from parliament.core.models import Politician, ElectedMember, Party, Riding, Session
...
class ElectedMemberResource(ModelResource):
    ...
    politician = fields.ToOneField('parliament.api.v1.PoliticianResource', 'politician')
    class Meta:
...
class PoliticianResource(BaseResource):
    ...
    elected_members = fields.ToManyField(ElectedMemberResource, 'electedmember_set', full=True)
    ...
    def override_urls(self):
        return [
            url(r"^(?P<resource_name>%s)/parliament_id/(?P<parl_id>\d+)/$" % self._meta.resource_name, self.wrap_view('dispatch_custom')),
        ]
        
    def get_custom(self, request, **kwargs):
        if 'parl_id' in kwargs:
            try:
                obj = Politician.objects.get_by_parl_id(int(kwargs['parl_id']))
...
class VoteQuestionResource(ModelResource):
    ...
    def override_urls(self):
        ...
        return [
            url(r"^(?P<resource_name>%s)/(?P<session>\d+-\d)/(?P<number>\d+)/$" % self._meta.resource_name, self.wrap_view('dispatch_detail'), name="api_dispatch_detail"),
        ]
        
class BillResource(ModelResource):
    sessions = fields.ToManyField(SessionResource, 'sessions')
    votequestions = fields.ToManyField(VoteQuestionResource, 'votequestion_set')
