...
r_politicalpost = re.compile(
    r'(Minister|Leader|Secretary|Solicitor|Attorney|Speaker|Deputy ' \
    '|Soliciter|Chair |Parliamentary|President |for )'
)

r_honorific = re.compile(
    r'^(Mr\.?|Mrs\.?|Ms\.?|Miss\.?|Hon\.?|Right Hon\.|The|A|An\.?|Some' \
    '|M\.|One|Santa|Acting|L\'hon\.|Assistant|Mme)\s(.+)$',
...
def time(hour, minute):
    if hour >= 24:
        hour = hour % 24
        # no, really. the house of commons is so badass they meet at 25 o'clock
...
def time_to_datetime(hour, minute, date):
    ...
    Necessary to deal with the occasional wacky 25 o'clock
    timestamps in Hansard."""
    if hour < 24:
...
def normalizeHansardURL(u):
    ...
    return 'http://www2.parl.gc.ca/HousePublications/' \
        'Publication.aspx?Language=E&Mode=1&Parl=%s&Ses=%s&DocId=%s' \
        % (parl, ses, docid)
...
def etree_extract_text(elem):
    ...
    for x in elem.getiterator():
        if text and x.tag in ('Para', 'P', 'p'):
            text += "\n\n"
        text += (
            none_to_empty(x.text) + none_to_empty(x.tail)
        ).replace("\n", ' ')
    return text
