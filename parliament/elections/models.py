...
class Election (models.Model):
    ...
    def __unicode__(self):
        if self.byelection:
            return _(u"Byelection of %s") % self.date
        else:
            return _(u"General election of %s") % self.date
    ...
    def calculate_vote_percentages(self):
        ...
        for riding in riding_candidacies:
            for candidacy in riding_candidacies[riding]:
                candidacy.votepercent = (
                    Decimal(candidacy.votetotal) / riding_votetotals[riding]
                ) * 100
                candidacy.save()
    ...
    def create_from_name(self, first_name, last_name, riding, party, election,
        ...
        if not candidates:
            ...
            if first_name.strip().count(' ') >= 1:
                ...
                candidates = Politician.objects.filter_by_name(
                    "%s %s" % (minifirst, last_name)
                )
        # Then, evaluate the possibilities in the list
        for posscand in candidates:
            # You're only a match if you've run for office for
            #the same party in the same province
            match = ElectedMember.objects.filter(
                riding__province=riding.province,
                party=party,
                politician=posscand
            ).count() >= 1 or Candidacy.objects.filter(
                riding__province=riding.province,
                party=party,
                candidate=posscand
            ).count() >= 1
            if match:
                if candidate is not None:
                    if interactive:
                        print _("Please enter Politician ID for " \
                            "%(fullname)r (%(riding)r)") % {
                            fullname: fullname,
                            riding: riding.name
                        }
                        candidate = Politician.objects.get(
                            pk=raw_input().strip()
                        )
                        break
                    else:
                        raise Politician.MultipleObjectsReturned(
                            _("Could not disambiguate among existing " \
                            "candidates for %s") % fullname
                        )
                candidate = posscand
...
class Candidacy (models.Model):
    ...
    votepercent = models.DecimalField(
        max_digits=5,
        decimal_places=2,
        blank=True,
        null=True
    ...
    def create_member(self, session=None):
        ...
        if not self.elected:
            return False
        try:
            member = ElectedMember.objects.filter(
                models.Q(end_date__isnull=True)
                | models.Q(end_date=self.election.date)
            ).get(
    ...
    def __unicode__(self):
        return _(u"%(candidate)s (%(party)s) was a candidate in " \
            "%(riding)s in the %(election)s") % {
