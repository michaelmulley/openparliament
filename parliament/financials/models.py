...
class Contribution (models.Model):
    ...
    amount_monetary = models.DecimalField(
        max_digits=7,
        decimal_places=2,
        blank=True,
        null=True
    )
    amount_nonmonetary = models.DecimalField(
        max_digits=7,
        decimal_places=2,
        blank=True,
        null=True
    ...
    def __unicode__(self):
        return _(u"%(contributor)s contributed %(amount)s " \
          "to %(candidate)s (%(party)s) on %(date)s") % {
            contributor: self.contributor.name,
            amount: self.amount,
            candidate: self.individual_recipient.candidate,
            party: self.individual_recipient.party,
            date: self.date
        }
