...
def bill_pk_redirect(request, bill_id):
    ...
    return HttpResponsePermanentRedirect(
        urlresolvers.reverse(
            ...
            kwargs={
                'session_id': bill.get_session().id,
...
def bill(request, session_id, bill_number):
    ...
    c = RequestContext(request, {
        ...
        'title': (_('Bill %s') % bill.number)
            + (_(' (Historical)') if bill.session.end else ''),
...
def index(request):
    ...
    votes = VoteQuestion.objects.select_related('bill')\
        .filter(session=sessions[0])[:6]
...
def bills_for_session(request, session_id):
    ...
    return object_list(
        ...
        extra_context={
            'session': session,
            'votes': votes,
            'title': _('Bills for the %s') % session
        }
    )
...
def votes_for_session(request, session_id):
    ...
    return object_list(
        request,
        queryset=VoteQuestion.objects.select_related(depth=1)\
            .filter(session=session),
        extra_context={
            'session': session,
            'title': _('Votes for the %s') % session
        }
    )
...
class BillListFeed(Feed):
    title = _('Bills in the House of Commons')
    description = _('New bills introduced to the House, ' \
        'from openparliament.ca.')
    link = "/bills/"

    def items(self):
        return Bill.objects.filter(introduced__isnull=False)\
            .order_by('-introduced', 'number_only')[:25]

    def item_title(self, item):
        return _("Bill %(number)s (%(private)s)") % {
            number: item.number,
            private: _("Private member's") \
                if item.privatemember else _("Government")
        }
...
class BillFeed(Feed):
    ...
    def title(self, bill):
        return _("Bill %s") % bill.number

    def link(self, bill):
        return "http://openparliament.ca" + bill.get_absolute_url()

    def description(self, bill):
        return _("From openparliament.ca, " \
                "speeches about Bill %(number)s, %(name)s") % {
            number: bill.number,
            name: bill.name
        }
    ...
    def item_title(self, item):
        if isinstance(item, VoteQuestion):
            return _("Vote #%(number)s (%(result)s)") % {
                number: item.number,
                result: item.get_result_display()
            }
        else:
            return _("%(name)s (%(party)s%(date)s)") % {
                'name': item.name_info['display_name'],
                'party': item.member.party.short_name + '; ' \
                    if item.member else '',
